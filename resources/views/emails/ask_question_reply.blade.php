...
<div class="mail-main">
    <div class="container">
        <div class="mail-description" @if($lang=='ar') style="direction: rtl; text-align: right" @endif>
            ...
            <h5>{{trans('global.answer_on_your_question')}}. </h5>
            <h5><b>{{$question->question}}. </b></h5>
            <p>{{$question->details}}. </p><br/>
            <p>{{trans('global.the_answer')}}:<br/>{{$ask_inputs['answer']}}</p><br/>
            <p><a href="{{route('ask', [$question->id, $question->slug])}}" target="_blank">{{route('ask', [$question->id, $question->slug])}}</a></p><br/>
