...
<div class="mail-main">
    <div class="container">
        <div class="mail-description" @if($lang=='ar') style="direction: rtl; text-align: right" @endif>
            <h2>{{trans('global.hello')}}, </h2>
            <p>{{trans('global.reply_on_your_answer')}}. </p>
            <h5><b>{{$answer->question->question}}. </b></h5>
            <p>{{$answer->question->details}}. </p><hr/>
            <p>{{trans('global.the_answer')}}:<br/>{{$answer->answer}}</p><hr/>
            <p>{{trans('global.the_reply')}}:<br/>{{$data['answer']}}</p><hr/>
            <p><a href="{{route('ask', [$answer->question->id, $answer->question->slug])}}" target="_blank">{{route('ask', [$answer->question->id, $answer->question->slug])}}</a></p><br/>
            <p>{{trans('global.thank_you')}}</p>
        </div>
