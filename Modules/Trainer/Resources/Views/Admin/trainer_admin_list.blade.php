...
@section('list_add_button')
    <a href="{{route('createTrainer')}}" class="btn btn-lg btn-success">{{trans('admin.add')}} Trainer</a>
     @if(request('trashed'))
            <a href="{{route('listTrainer')}}" class="btn btn-lg btn-info">{{trans('admin.enabled')}}</a>
        @else
            <a href="{{route('listTrainer')}}?trashed=1" class="btn btn-lg btn-danger">{{trans('admin.disabled')}}</a>
        @endif
    ...
    <div class="row">
        ...
        @foreach($trainers as $key=> $trainer)
            <tr>
                ...
                <td> {{ $trainer->id }}</td>
                <td> {{ $trainer->name }}</td>
                <td> <img src="{{ $trainer->image }}" style="height: 60px;object-fit: cover;"></td>
                <td> {{ $trainer->views }}</td>
                <td> {!! $trainer->published == '1' ? '<span style="color:green"><i class="fa fa-check"></i></span>' : '<span style="color:red"><i class="fa fa-times"></i></span>' !!} </td>
                <td>

                    <a href="{{route('trainer',[$trainer->id, $trainer->slug])}}" target="_blank" class="btn btn-xs yellow">
                        <i class="fa fa-eye"></i>
                    </a>
                    <a href="{{route('editTrainer',$trainer->id)}}" class="btn btn-xs yellow">
                        <i class="fa fa-edit"></i>
                    </a>
                    @if(request('trashed'))
                        <a title="{{trans('admin.enable')}}" href="{{route('deleteTrainer',$trainer->id)}}" class="confirm_delete btn btn-xs green">
                            <i class="fa fa-check-circle"></i>
                        </a>
                    @else
                        <a title="{{trans('admin.disable')}}" href="{{route('deleteTrainer',$trainer->id)}}" class="confirm_delete btn btn-xs red">
