...
@section('list_add_button')
    <a href="{{route('createTrainingSubscription', $user)}}" class="btn btn-lg btn-success">{{trans('admin.add')}} Client</a>
     @if(request('trashed'))
            <a href="{{route('listTrainingSubscription', $user)}}" class="btn btn-lg btn-info">{{trans('admin.enabled')}}</a>
        @else
            <a href="{{route('listTrainingSubscription', $user)}}?trashed=1" class="btn btn-lg btn-danger">{{trans('admin.disabled')}}</a>
        @endif
    ...
    <div class="row">
        ...
        <form action="" id="filter_form">
            <table class="table table-striped table-bordered table-hover ">
                ...
                <tr>
                    ...
                    <td><input id="date" value="{{ request('date')}}" name="date" class="form-control"
                               type="date" placeholder="{{trans('admin.date')}}"/></td>
                </tr>
                </tbody>
            </table>
        ...
        @foreach($clients as $key=> $client)
            <tr>
                <td> {{ $client->id }}</td>
                <td> {{ $client->title }}</td>
                <td> <img src="{{ $client->image }}" style="width: 120px;height: 120px;object-fit: contain" /></td>
                <td> {{ \Carbon\Carbon::parse($client->created_at)->toDateString() }}</td>
                <td>
                    <a href="{{route('editTrainingSubscription', [$user, $client->id])}}" class="btn btn-xs yellow">
                        <i class="fa fa-edit"></i>
                    </a>
                    @if(request('trashed'))
                        <a title="{{trans('admin.enable')}}" href="{{route('deleteTrainingSubscription',[$user, $client->id])}}" class="confirm_delete btn btn-xs green">
                            <i class="fa fa-check-circle"></i>
                        </a>
                    @else
                        <a title="{{trans('admin.disable')}}" href="{{route('deleteTrainingSubscription',[$user, $client->id])}}" class="confirm_delete btn btn-xs red">
