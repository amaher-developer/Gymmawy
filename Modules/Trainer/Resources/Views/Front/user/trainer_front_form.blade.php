...
@section('breadcrumb')
    <ul class="page-breadcrumb breadcrumb">
        ...
        <li>
            <a href="{{ route('showUserTrainer') }}">{{trans('admin.trainer_show')}}</a>
            <i class="fa fa-circle"></i>
        </li>
        <li>
            {{ $title }}
...
@section('page_body')
    ...
    <form method="post" action="" name="myForm" id="myForm" class="form-horizontal" role="form" enctype="multipart/form-data">
        <div class="form-body">
            {{csrf_field()}}

            <h5 class="form-section">{{trans('admin.personal_info')}}</h5>
            <div class="form-group col-md-12  col-sm-12">
                <label class="col-md-3  col-sm-3 control-label">{{trans('admin.name')}} <span class="required">*</span></label>
                <div class="col-md-9  col-sm-9">
                    <input id="name" value="{{ old('name_'.$lang, @$trainer['name_'.$lang]) }}"
                           name="name_{{$lang}}" type="text" class="form-control" required>
                </div>
            </div>
            <div class="form-group col-md-12  col-sm-12">
                <label class="col-md-3  col-sm-3 control-label">{{trans('admin.description')}} <span class="required">*</span></label>
                <div class="col-md-9  col-sm-9">
                    <textarea id="about"
                              name="about_{{$lang}}"  class="form-control" required>{{ old('about_'.$lang,  @$trainer['about_'.$lang] ) }}</textarea>
                </div>
            </div>
            ...
            <div class="form-group col-md-12 col-sm-12">
                ...
                <div class="col-md-8  col-sm-8">
                    <input id="image" value="{{ old('image', @$trainer->image) }}"
                           name="image" type="file" class="form-control" @if(@!$trainer->image) required @endif>
                    <br/>
            ...
            <div class="form-group col-md-12 col-sm-12">
                <label class="col-md-3 col-sm-3 control-label">{{trans('admin.linkedin')}}</label>
                <div class="col-md-9 col-sm-9">
                    <input id="linkedin" value="{{ old('linkedin', @$trainer->linkedin) }}" dir="ltr"
                                ...
                                <div class="form-group">
                                    <div class="col-sm-offset-1 col-sm-10">
                                        <label class="mt-checkbox mt-checkbox-outline">
                                            <input type="checkbox" class="categories" name="categories[]" value="{{$category->id}}" @if(in_array($category->id, $trainer_category_ids)) checked @endif required> {{$category->name}}
                                            <span></span>
                                        </label>
                                    </div>
                                </div>
...
{{--            </div>--}}
            ...
            <div class="form-group col-md-12  col-sm-12" style="clear:both;"></div>
...
{{--                                   @if(@$gym->published) {{ $gym->published?'checked':'0' }} @else checked @endif>--}}
{{--                            <span></span>--}}
{{--                        </label>--}}
{{--                    </div>--}}
{{--                </div>--}}
