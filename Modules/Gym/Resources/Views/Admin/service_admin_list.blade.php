...
@section('list_add_button')
    <a href="{{route('createService')}}" class="btn btn-lg btn-success">{{trans('admin.add')}} Service</a>
     @if(request('trashed'))
            <a href="{{route('listService')}}" class="btn btn-lg btn-info">{{trans('admin.enabled')}}</a>
        @else
            <a href="{{route('listService')}}?trashed=1" class="btn btn-lg btn-danger">{{trans('admin.disabled')}}</a>
        @endif
...
@section('page_body')
    <div class="row">
        ...
        <tr class="">
            <th>#</th>
            <th>id</th><th>name</th><th>logo</th>
        ...
        @foreach($services as $key=> $service)
            <tr>
                ...
                <td><img src="{{ $service->logo }}"></td>
                <td>
                    <a href="{{route('editService',$service->id)}}" class="btn btn-xs yellow">
                        <i class="fa fa-edit"></i>
                    </a>
                    @if(request('trashed'))
                        <a title="{{trans('admin.enable')}}" href="{{route('deleteService',$service->id)}}" class="confirm_delete btn btn-xs green">
                            <i class="fa fa-check-circle"></i>
                        </a>
                    @else
                        <a title="{{trans('admin.disable')}}" href="{{route('deleteService',$service->id)}}" class="confirm_delete btn btn-xs red">
     ...
     <div class="col-lg-5 col-md-5 col-md-offset-5">
                {!! $services->appends($search_query)->render()  !!}
            </div>
        </div>
@endsection
