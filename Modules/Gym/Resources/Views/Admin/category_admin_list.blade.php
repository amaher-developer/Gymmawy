...
@section('list_add_button')
    <a href="{{route('createCategory')}}" class="btn btn-lg btn-success">{{trans('admin.add')}} Category</a>
     @if(request('trashed'))
            <a href="{{route('listCategory')}}" class="btn btn-lg btn-info">{{trans('admin.enabled')}}</a>
        @else
            <a href="{{route('listCategory')}}?trashed=1" class="btn btn-lg btn-danger">{{trans('admin.disabled')}}</a>
        @endif
        ...
        @foreach($categories as $key=> $category)
            <tr>
                ...
                <td>
                    ...
                    @if(request('trashed'))
                        <a title="{{trans('admin.enable')}}" href="{{route('deleteCategory',$category->id)}}" class="confirm_delete btn btn-xs green">
                            <i class="fa fa-check-circle"></i>
                        </a>
                    @else
                        <a title="{{trans('admin.disable')}}" href="{{route('deleteCategory',$category->id)}}" class="confirm_delete btn btn-xs red">
