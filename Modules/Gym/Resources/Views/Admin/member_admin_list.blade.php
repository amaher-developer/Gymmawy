...
@section('list_add_button')
    <a href="{{route('createMember')}}" class="btn btn-lg btn-success">{{trans('admin.add')}} Member</a>
     @if(request('trashed'))
            <a href="{{route('listMember')}}" class="btn btn-lg btn-info">{{trans('admin.enabled')}}</a>
        @else
            <a href="{{route('listMember')}}?trashed=1" class="btn btn-lg btn-danger">{{trans('admin.disabled')}}</a>
        @endif
    ...
    <div class="row">
        ...
        @foreach($members as $key=> $member)
            <tr>
                ...
                <td>
                    <a href="{{route('editMember',$member->id)}}" class="btn btn-xs yellow">
                        <i class="fa fa-edit"></i>
                    </a>
                    @if(request('trashed'))
                        <a title="{{trans('admin.enable')}}" href="{{route('deleteMember',$member->id)}}" class="confirm_delete btn btn-xs green">
                            <i class="fa fa-check-circle"></i>
                        </a>
                    @else
                        <a title="{{trans('admin.disable')}}" href="{{route('deleteMember',$member->id)}}" class="confirm_delete btn btn-xs red">
