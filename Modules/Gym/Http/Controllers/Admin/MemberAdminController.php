...
use Modules\Gym\Http\Requests\GymMemberRequest;
use Modules\Gym\Repositories\GymMemberRepository;
use Carbon\Carbon;
use Maatwebsite\Excel\Facades\Excel;
use Illuminate\Support\Facades\File;
use Intervention\Image\Facades\Image;

class MemberAdminController extends GenericAdminController
{
    ...
    public function index()
    {
        ...
        if(request('trashed'))
        {
            $members = $this->MemberRepository->onlyTrashed()->orderBy('id', 'DESC');
        }
        else
        {
            $members = $this->MemberRepository->orderBy('id', 'DESC');
        }


             //apply filters
                $members->when($id, function ($query) use ($id) {
                        $query->where('id','=', $id);
                });
                 $search_query = request()->query();
                       ...
                       if (request()->ajax() && request()->exists('export')) {
                             ...
                             $fileName = 'members-' . Carbon::now()->toDateTimeString();
        ...
        return view('gym::Admin.member_admin_list', compact('members','title', 'total', 'search_query'));
    }
    ...
    {
        $title = 'Create Member';
        return view('gym::Admin.member_admin_form', ['member' => new Member(),'title'=>$title]);
    }

    public function store(GymMemberRequest $request)
    {
        $member_inputs = $this->prepare_inputs($request->except(['_token']));
        $this->MemberRepository->create($member_inputs);
        sweet_alert()->success('Done', 'Member Added successfully');
        return redirect(route('listMember'));
    }

    public function edit($id)
    {
        $member =$this->MemberRepository->withTrashed()->find($id);
        $title = 'Edit Member';
        return view('gym::Admin.member_admin_form', ['member' => $member,'title'=>$title]);
    }
    ...
    public function update(GymMemberRequest $request, $id)
    {
        ...
        sweet_alert()->success('Done', 'Member Updated successfully');
        return redirect(route('listMember'));
    }
    ...
    public function destroy($id)
      {
        ...
        sweet_alert()->success('Done', 'Member Deleted successfully');
        return redirect(route('listMember'));
    }
    ...
    private function prepare_inputs($inputs)
    {
        ...
        $uploaded='';
                ...
                $destinationPath = base_path($this->MemberRepository->model()::$uploads_path);
                $ThumbnailsDestinationPath = base_path($this->MemberRepository->model()::$thumbnails_uploads_path);
