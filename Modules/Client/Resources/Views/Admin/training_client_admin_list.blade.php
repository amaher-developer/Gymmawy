...
@section('list_add_button')
    <a href="{{route('createTrainingClient')}}" class="btn btn-lg btn-success">{{trans('admin.add')}} Client</a>
     @if(request('trashed'))
            <a href="{{route('listTrainingClient')}}" class="btn btn-lg btn-info">{{trans('admin.enabled')}}</a>
        @else
            <a href="{{route('listTrainingClient')}}?trashed=1" class="btn btn-lg btn-danger">{{trans('admin.disabled')}}</a>
        @endif
    ...
    <div class="row">
        ...
        @foreach($clients as $key=> $client)
            <tr>
                ...
                <td>
                    <a href="{{route('listTrainingSubscription',$client->id)}}" class="btn btn-xs yellow">
                        <i class="fa fa-list"></i>
                    </a>
                    <a href="{{route('editTrainingClient',$client->id)}}" class="btn btn-xs yellow">
                        <i class="fa fa-edit"></i>
                    </a>
                    @if(request('trashed'))
                        <a title="{{trans('admin.enable')}}" href="{{route('deleteTrainingClient',$client->id)}}" class="confirm_delete btn btn-xs green">
                            <i class="fa fa-check-circle"></i>
                        </a>
                    @else
                        <a title="{{trans('admin.disable')}}" href="{{route('deleteTrainingClient',$client->id)}}" class="confirm_delete btn btn-xs red">
