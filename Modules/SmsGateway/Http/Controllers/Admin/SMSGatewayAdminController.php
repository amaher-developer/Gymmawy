...
class SMSGatewayAdminController extends GenericAdminController
{
    public static $gateway = 'https://platform.clickatell.com/';
    public static $password = 'sZf7yzz5U3';
    public static $username = 'SAU73396814843';
    public static $sender = 'Yacune';
    public static $apiKey = '6EIcJHvXQWatBJYYcflqqg==';


    public static function sentSms($to = null, $content = 'hello'){
        $apiKey = self::$apiKey;
        ...
        if($to && $content) {
                ...
                $mob = $to;
                if(substr($mob, 0, 2) == "01") $to = str_replace("01", "201", substr($mob,0, 2)).substr($mob,2);
                elseif(substr($mob, 0, 2) == "05") $to = str_replace("05", "9665", substr($mob,0, 2)).substr($mob,2);
                else $to = $mob;

                $to = ltrim($to, 0);
                $url = self::$gateway.'messages/http/send?apiKey=' . $apiKey . '&to=' . $to.'&content=' .  urlencode($content) ;
    ...
    public function sentSmsTest(){
        ...
        if($to && $content) {
            ...
            $url = $this->gateway.'messages/http/send?apiKey=' . $apiKey . '&to=' . $to.'&content=' .  urlencode($content) ;
...
//                else $t = $mob;
//
//                $url = 'https://platform.clickatell.com/messages/http/send?apiKey=' . $apiKey . '&to=' . $t . '&content=' . urlencode($content);
//
//                $ch = curl_init($url);
//
//                curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json']);
...
//    public static function sentSms($to = null, $content = 'hello', $apiKey = '60a2250289a88daae298414623d9d7ca'){
////        $to = request('to');
////        $content = request('msg');
//        if($to && $content) {
//
...
//
//          //  echo $message.'<Br/>';
//            $message = (self::convertToUnicode($message));
//         //   echo $message.'<br/>';
//            $timeSend = 0;
//            $dateSend = 0;
//            $deleteKey = 152485;
...
//         //   echo '<br/>'.'To: '.$to.'<br/>';
//            // auth call
////            $url = "$baseurl/api/msgSend.php?mobile=$mobile&password=$password&numbers=$to&sender=$sender&msg=$message&timeSend=$timeSend&dateSend=$dateSend&applicationType=24&domainName=localhost";
//            $url = "$baseurl/api/msgSend.php?apiKey=$apiKey&numbers=$to&sender=$sender&msg=$message&timeSend=$timeSend&dateSend=$dateSend&applicationType=24&domainName=localhost";
//
//            // do auth call
//            $ret = file($url);
////            echo $url.'<br/>';
...
//    }
    ...
    static public function convertToUnicode($message)
    {
        $chrArray[0] = "،";
        $unicodeArray[0] = "060C";
        $chrArray[1] = "؛";
