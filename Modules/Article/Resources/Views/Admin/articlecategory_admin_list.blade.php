...
@section('list_add_button')
    <a href="{{route('createArticleCategory')}}" class="btn btn-lg btn-success">{{trans('admin.add')}} ArticleCategory</a>
     @if(request('trashed'))
            <a href="{{route('listArticleCategory')}}" class="btn btn-lg btn-info">{{trans('admin.enabled')}}</a>
        @else
            <a href="{{route('listArticleCategory')}}?trashed=1" class="btn btn-lg btn-danger">{{trans('admin.disabled')}}</a>
        @endif
...
@section('page_body')
    ...
    <table class="table table-striped table-bordered table-hover" >
        ...
        @foreach($articlecategories as $key=> $articlecategory)
            <tr>
                <td> {{ $key+1 }}</td>
                <td> {{ $articlecategory->id }}</td><td> {{ $articlecategory->name }}</td>
                <td>
                    <a href="{{route('editArticleCategory',$articlecategory->id)}}" class="btn btn-xs yellow">
                        <i class="fa fa-edit"></i>
                    </a>
                    @if(request('trashed'))
                        <a title="{{trans('admin.enable')}}" href="{{route('deleteArticleCategory',$articlecategory->id)}}" class="confirm_delete btn btn-xs green">
                            <i class="fa fa-check-circle"></i>
                        </a>
                    @else
                        <a title="{{trans('admin.disable')}}" href="{{route('deleteArticleCategory',$articlecategory->id)}}" class="confirm_delete btn btn-xs red">
     ...
     <div class="col-lg-5 col-md-5 col-md-offset-5">
                {!! $articlecategories->appends($search_query)->render()  !!}
            </div>
        </div>
@endsection
