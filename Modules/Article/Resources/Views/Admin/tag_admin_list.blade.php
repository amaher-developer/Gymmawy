...
@section('list_add_button')
    <a href="{{route('createTag')}}" class="btn btn-lg btn-success">{{trans('admin.add')}} Tag</a>
     @if(request('trashed'))
            <a href="{{route('listTag')}}" class="btn btn-lg btn-info">{{trans('admin.enabled')}}</a>
        @else
            <a href="{{route('listTag')}}?trashed=1" class="btn btn-lg btn-danger">{{trans('admin.disabled')}}</a>
        @endif
...
@section('page_body')
    ...
    <table class="table table-striped table-bordered table-hover" >
        ...
        @foreach($tags as $key=> $tag)
            <tr>
                <td> {{ $key+1 }}</td>
                <td> {{ $tag->id }}</td><td> {{ $tag->name }}</td>
                <td>
                    <a href="{{route('editTag',$tag->id)}}" class="btn btn-xs yellow">
                        <i class="fa fa-edit"></i>
                    </a>
                    @if(request('trashed'))
                        <a title="{{trans('admin.enable')}}" href="{{route('deleteTag',$tag->id)}}" class="confirm_delete btn btn-xs green">
                            <i class="fa fa-check-circle"></i>
                        </a>
                    @else
                        <a title="{{trans('admin.disable')}}" href="{{route('deleteTag',$tag->id)}}" class="confirm_delete btn btn-xs red">
     ...
     <div class="col-lg-5 col-md-5 col-md-offset-5">
                {!! $tags->appends($search_query)->render()  !!}
            </div>
        </div>
@endsection
