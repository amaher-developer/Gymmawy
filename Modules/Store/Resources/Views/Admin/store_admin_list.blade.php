...
@section('list_add_button')
    <a href="{{route('createStore')}}" class="btn btn-lg btn-success">{{trans('admin.add')}} Store</a>
     @if(request('trashed'))
            <a href="{{route('listStore')}}" class="btn btn-lg btn-info">{{trans('admin.enabled')}}</a>
        @else
            <a href="{{route('listStore')}}?trashed=1" class="btn btn-lg btn-danger">{{trans('admin.disabled')}}</a>
        @endif
...
@section('page_body')
    <div class="row">
        ...
        @foreach($stores as $key=> $store)
            <tr>
                <td> {{ $key+1 }}</td>
                <td> {{ $store->id }}</td><td> {{ $store->name }}</td><td> {{ $store->price }}</td><td> {{ $store->quantity }}</td>
                <td>
                    <a href="{{route('editStore',$store->id)}}" class="btn btn-xs yellow">
                        <i class="fa fa-edit"></i>
                    </a>
                    @if(request('trashed'))
                        <a title="{{trans('admin.enable')}}" href="{{route('deleteStore',$store->id)}}" class="confirm_delete btn btn-xs green">
                            <i class="fa fa-check-circle"></i>
                        </a>
                    @else
                        <a title="{{trans('admin.disable')}}" href="{{route('deleteStore',$store->id)}}" class="confirm_delete btn btn-xs red">
     ...
     <div class="col-lg-5 col-md-5 col-md-offset-5">
                {!! $stores->appends($search_query)->render()  !!}
            </div>
        </div>
@endsection
