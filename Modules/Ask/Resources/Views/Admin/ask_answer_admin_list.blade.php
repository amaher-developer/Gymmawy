...
@section('list_add_button')
    <a href="{{route('listAsk')}}" class="btn btn-lg btn-success">{{trans('admin.questions')}}</a>
{{--     @if(request('trashed'))--}}
    ...
    <div class="row">
        ...
        @foreach($asks as $key=> $ask)
            <tr>
                <td> {{ $ask->id }}</td>
                <td> {{ $ask->answer }}
                    <br/> <span style="font-size: 10px;color: grey"><b>{{trans('admin.question')}}</b>: {{@$ask->question->question}}</span>
                    @if(@$ask->parent_answer->answer)<br/> <span style="font-size: 10px;color: grey"><b>{{trans('admin.parent_answer')}}</b>: {{@$ask->parent_answer->answer}}</span>@endif
                </td>
                <td>{{ $ask->created_at }}</td>
                <td>
                    <a href="{{route('editAskAnswer',$ask->id)}}" class="btn btn-xs yellow">
                        <i class="fa fa-edit"></i>
                    </a>
                    @if(request('trashed'))
                        <a title="{{trans('admin.enable')}}" href="{{route('deleteAskAnswer',$ask->id)}}" class="confirm_delete btn btn-xs green">
                            <i class="fa fa-check-circle"></i>
                        </a>
                    @else
                        <a title="{{trans('admin.disable')}}" href="{{route('deleteAskAnswer',$ask->id)}}" class="confirm_delete btn btn-xs red">
