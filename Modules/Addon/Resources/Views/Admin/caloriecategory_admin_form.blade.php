...
@section('breadcrumb')
    <ul class="page-breadcrumb breadcrumb">
        ...
        <li>
            <a href="{{ route('listCalorieCategory') }}">CalorieCategories</a>
             <i class="fa fa-circle"></i>
        </li>
        <li>{{ $title }}</li>
    </ul>
...
@section('page_body')
    ...
    <div class="col-md-9">
        <input id="name_en" value="{{ old('name_en', $caloriecategory->name_en) }}"
    ...
    <div class="col-md-9">
        <input id="name_ar" value="{{ old('name_ar', $caloriecategory->name_ar) }}"
    ...
    <div class="col-md-8">
        <input id="image" value="{{ old('image', $caloriecategory->image) }}"
               name="image" type="file" class="form-control" >
    </div>
    @if(!empty($caloriecategory->image))
       <label class="col-md-1 control-label">
            <a href="{{ $caloriecategory->image }}" class="fancybox-button" data-rel="fancybox-button">
                view
            </a>
       </label>
    @endif
            ...
            <div class="mt-checkbox-list">
                <label class="mt-checkbox mt-checkbox-outline">
                    ...
                    <input type="checkbox" value="{{ date('Y-m-d') }}" name="deleted_at"
                            {{ $caloriecategory->trashed()?'checked':'' }}>
                    <span></span>
                </label>
            </div>
        </div>
